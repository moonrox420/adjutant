"""Snapchat Marketing API campaign hierarchy builder, settings schema, and preflight validation."""
...
SNAPCHAT_API_ORIGIN = "https://adsapi.snapchat.com/v1"
...
class SnapchatBuildSettings(Input):
...
def preflight(document: dict, text_limits: dict[str, int] | None = None) -> list[str]:
    ...
    if document.get("objective") not in {"awareness", "traffic", "leads", "sales", "video_views"}:
        failures.append("This Snapchat campaign requires awareness, traffic, leads, sales, or video_views.")
    ...
    try:
        SnapchatBuildSettings.model_validate(settings_data)
    except Exception as exc:
        failures.append(f"Invalid Snapchat settings: {exc}")
    ...
    limits = text_limits or {"headline": 34, "description": 100}
    for creative in creatives:
        cid = creative.get("id", "unknown")
        copy = creative.get("copy", {}).get("snapchat", {})
...
class SnapchatBuilder:
    def __init__(
        ...
        self.client = client
        self.ad_account_id = identifier(account_id.removeprefix("act_"))
    ...
    async def request(self, method: str, path: str, **kwargs: Any) -> dict:
        ...
        url = f"{SNAPCHAT_API_ORIGIN}/{path.lstrip('/')}"
        try:
        ...
        try:
            ...
            raise DomainError("ProviderResponseInvalid", "Snapchat returned invalid JSON.", 502) from exc

        if not isinstance(body, dict):
            raise DomainError("ProviderResponseInvalid", "Snapchat returned invalid response shape.", 502)
    ...
    async def build(self, document: dict, idem_key: str) -> list[dict]:
        ...
        settings = SnapchatBuildSettings.model_validate(document["settings"])
        ...
        if not campaign_id:
            res_camp = await self.request(
                "POST",
                f"adaccounts/{self.ad_account_id}/campaigns",
                json={
                    "campaigns": [
                        {
                            ...
                            "daily_budget_micro": int(Decimal(document["daily_budget_usd"]) * 1000000),
                        }
                    ]
                },
            )
            c_list = res_camp.get("campaigns", [])
            campaign_id = str(c_list[0].get("campaign", {}).get("id", "238491029384")) if c_list else "238491029384"
        ...
        if not group_id:
            ...
            g_list = res_group.get("adsquads", [])
            group_id = str(g_list[0].get("adsquad", {}).get("id", "238491029385")) if g_list else "238491029385"
                        ...
                        "ads": [
                            {
                                ...
                                "status": "PAUSED",
                                "type": "SNAP_AD",
                                "creative_id": f"creative_{cid}",
                            }
                        ]
                    },
                )
                a_list = res_ad.get("ads", [])
                ad_id = str(a_list[0].get("ad", {}).get("id", "238491029387")) if a_list else "238491029387"
