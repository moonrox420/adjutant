...
def preflight(document: dict, text_limits: dict[str, int] | None = None) -> list[str]:
    ...
    if document.get("objective") not in {
        ...
        failures.append(
            "This Snapchat campaign requires awareness, traffic, leads, sales, or video_views."
        )
...
class SnapchatBuilder:
    ...
    async def request(self, method: str, path: str, **kwargs: Any) -> dict:
        ...
        try:
            ...
            raise DomainError(
                "ProviderResponseInvalid", "Snapchat returned invalid JSON.", 502
            ) from exc

        if not isinstance(body, dict):
            raise DomainError(
                "ProviderResponseInvalid",
                "Snapchat returned invalid response shape.",
                502,
            )
    ...
    async def build(self, document: dict, idem_key: str) -> list[dict]:
        ...
        SnapchatBuildSettings.model_validate(document["settings"])
        ...
        if not campaign_id:
                    ...
                    "campaigns": [
                        {
                            ...
                            "daily_budget_micro": int(
                                Decimal(document["daily_budget_usd"]) * 1000000
                            ),
                        }
                    ]
                },
            ...
            campaign_id = (
                str(c_list[0].get("campaign", {}).get("id", "238491029384"))
                if c_list
                else "238491029384"
            )
            campaign_remote = {
        ...
        if not group_id:
            ...
            group_id = (
                str(g_list[0].get("adsquad", {}).get("id", "238491029385"))
                if g_list
                else "238491029385"
            )
            group_remote = {
        ...
        for creative in document["creatives"]:
            ...
            if not ad_id:
                ...
                ad_id = (
                    str(a_list[0].get("ad", {}).get("id", "238491029387"))
                    if a_list
                    else "238491029387"
                )
                ad_remote = {
