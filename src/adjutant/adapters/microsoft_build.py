...
def preflight(document: dict, text_limits: dict[str, int] | None = None) -> list[str]:
    ...
    if document.get("objective") not in {
        ...
        failures.append(
            "This Microsoft Advertising campaign requires awareness, traffic, leads, sales, "
            "or video_views."
        )
...
class MicrosoftBuilder:
    ...
    async def request(self, method: str, path: str, **kwargs: Any) -> dict:
        ...
        try:
            ...
            raise DomainError(
                "ProviderStateUncertain", "Microsoft Advertising did not respond.", 503
            ) from exc
        ...
        try:
            ...
            raise DomainError(
                ...
                "Microsoft Advertising returned invalid JSON.",
                502,
            ) from exc
        ...
        if not isinstance(body, dict):
            raise DomainError(
                "ProviderResponseInvalid",
                "Microsoft Advertising returned invalid response shape.",
                502,
            )
