...
def preflight(document: dict, text_limits: dict[str, int] | None = None) -> list[str]:
    ...
    if document.get("objective") not in {
        ...
        failures.append(
            "This LinkedIn campaign requires awareness, traffic, leads, sales, or video_views."
        )
...
class LinkedInBuilder:
    ...
    async def request(self, method: str, path: str, **kwargs: Any) -> dict:
        ...
        try:
            ...
            raise DomainError(
                "ProviderResponseInvalid", "LinkedIn returned invalid JSON.", 502
            ) from exc

        if not isinstance(body, dict):
            raise DomainError(
                "ProviderResponseInvalid",
                "LinkedIn returned invalid response shape.",
                502,
            )
        ...
        if not campaign_id:
            res_camp = await self.request(
                ...
                json={
                    ...
                    "status": "PAUSED",
                    "totalBudget": {
                        "amount": str(Decimal(document["monthly_budget_usd"])),
                        "currencyCode": "USD",
                    },
                },
            )
        ...
        if not group_id:
            res_group = await self.request(
                ...
                json={
                    ...
                    "dailyBudget": {
                        "amount": str(Decimal(document["daily_budget_usd"])),
                        "currencyCode": "USD",
                    },
                },
            )
