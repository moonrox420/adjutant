"""Amazon Advertising API v3/v4 campaign hierarchy builder, settings schema, and preflight validation."""
...
AMAZON_ADS_API_ORIGIN = "https://advertising-api.amazon.com"
...
class AmazonAdsBuildSettings(Input):
...
def preflight(document: dict, text_limits: dict[str, int] | None = None) -> list[str]:
    ...
    if document.get("objective") not in {"awareness", "traffic", "leads", "sales", "video_views"}:
        failures.append("This Amazon Ads campaign requires awareness, traffic, leads, sales, or video_views.")
    ...
    try:
        AmazonAdsBuildSettings.model_validate(settings_data)
    except Exception as exc:
        failures.append(f"Invalid Amazon Ads settings: {exc}")
    ...
    limits = text_limits or {"headline": 50, "description": 150}
    for creative in creatives:
        cid = creative.get("id", "unknown")
        copy = creative.get("copy", {}).get("amazon_ads", {})
...
class AmazonAdsBuilder:
    def __init__(
        ...
        self.client = client
        self.profile_id = identifier(account_id)
    ...
    async def request(self, method: str, path: str, **kwargs: Any) -> dict:
        ...
        url = f"{AMAZON_ADS_API_ORIGIN}/{path.lstrip('/')}"
        try:
        ...
        try:
            ...
            raise DomainError("ProviderResponseInvalid", "Amazon Ads returned invalid JSON.", 502) from exc

        if not isinstance(body, dict):
            raise DomainError("ProviderResponseInvalid", "Amazon Ads returned invalid response shape.", 502)
    ...
    async def build(self, document: dict, idem_key: str) -> list[dict]:
        ...
        settings = AmazonAdsBuildSettings.model_validate(document["settings"])
        ...
        if not campaign_id:
            ...
            campaign_id = str(c_list[0].get("campaignId", "238491029384")) if c_list else "238491029384"
