"""Amazon Advertising API v3/v4 campaign hierarchy builder, settings schema,
...
def preflight(document: dict, text_limits: dict[str, int] | None = None) -> list[str]:
    ...
    if document.get("objective") not in {
        ...
        failures.append(
            "This Amazon Ads campaign requires awareness, traffic, leads, sales, or video_views."
        )
...
class AmazonAdsBuilder:
    ...
    async def request(self, method: str, path: str, **kwargs: Any) -> dict:
        ...
        try:
            ...
            raise DomainError(
                "ProviderResponseInvalid", "Amazon Ads returned invalid JSON.", 502
            ) from exc

        if not isinstance(body, dict):
            raise DomainError(
                "ProviderResponseInvalid",
                "Amazon Ads returned invalid response shape.",
                502,
            )
    ...
    async def build(self, document: dict, idem_key: str) -> list[dict]:
        ...
        AmazonAdsBuildSettings.model_validate(document["settings"])
