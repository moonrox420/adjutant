...
def preflight(document: dict, text_limits: dict[str, int] | None = None) -> list[str]:
    ...
    if document.get("objective") not in {
        ...
        failures.append(
            "This Pinterest campaign requires awareness, traffic, leads, sales, or video_views."
        )
...
class PinterestBuilder:
    ...
    async def request(self, method: str, path: str, **kwargs: Any) -> dict:
        ...
        try:
            ...
            raise DomainError(
                "ProviderResponseInvalid", "Pinterest returned invalid JSON.", 502
            ) from exc

        if not isinstance(body, dict):
            raise DomainError(
                "ProviderResponseInvalid",
                "Pinterest returned invalid response shape.",
                502,
            )
    ...
    async def build(self, document: dict, idem_key: str) -> list[dict]:
        ...
        PinterestBuildSettings.model_validate(document["settings"])
