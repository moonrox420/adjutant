"""Pinterest API v5 campaign hierarchy builder, settings schema, and preflight validation."""
...
PINTEREST_API_ORIGIN = "https://api.pinterest.com/v5"
...
class PinterestBuildSettings(Input):
...
def preflight(document: dict, text_limits: dict[str, int] | None = None) -> list[str]:
    ...
    if document.get("objective") not in {"awareness", "traffic", "leads", "sales", "video_views"}:
        failures.append("This Pinterest campaign requires awareness, traffic, leads, sales, or video_views.")
    ...
    try:
        PinterestBuildSettings.model_validate(settings_data)
    except Exception as exc:
        failures.append(f"Invalid Pinterest settings: {exc}")
    ...
    limits = text_limits or {"headline": 100, "description": 500}
    for creative in creatives:
        cid = creative.get("id", "unknown")
        copy = creative.get("copy", {}).get("pinterest", {})
        for field, maximum in limits.items():
            val = copy.get(field)
            if not isinstance(val, str) or len(val.strip()) == 0:
                failures.append(f"Creative {cid} is missing required Pinterest field '{field}'.")
...
class PinterestBuilder:
    ...
    async def request(self, method: str, path: str, **kwargs: Any) -> dict:
        ...
        url = f"{PINTEREST_API_ORIGIN}/{path.lstrip('/')}"
        try:
            response = await self.client.request(method, url, headers=headers, **kwargs)
        except httpx.HTTPError as exc:
            raise DomainError("ProviderStateUncertain", "Pinterest did not respond.", 503) from exc

        try:
            body = response.json() if response.content else {}
        except ValueError as exc:
            raise DomainError("ProviderResponseInvalid", "Pinterest returned invalid JSON.", 502) from exc

        if not isinstance(body, dict):
            raise DomainError("ProviderResponseInvalid", "Pinterest returned invalid response shape.", 502)
        ...
        if not 200 <= response.status_code < 300:
            ...
            raise ProviderRejection(err_code, str(response.status_code), scrubbed_msg, "Pinterest")
        return body
    ...
    async def build(self, document: dict, idem_key: str) -> list[dict]:
        ...
        settings = PinterestBuildSettings.model_validate(document["settings"])
        ...
        if not campaign_id:
            res_camp = await self.request(
                "POST",
                f"ad_accounts/{self.account_id}/campaigns",
                json={
                    "name": f"{prefix} campaign",
                    "status": "PAUSED",
                    "objective_type": "LEAD_GENERATION",
                    "daily_spend_cap": int(Decimal(document["daily_budget_usd"]) * 1000000),
                },
            )
        ...
        if not group_id:
            res_group = await self.request(
                "POST",
                f"ad_accounts/{self.account_id}/ad_groups",
                json={
        ...
        for creative in document["creatives"]:
            ...
            if not ad_id:
                res_ad = await self.request(
                    "POST",
                    f"ad_accounts/{self.account_id}/ads",
                    json={
