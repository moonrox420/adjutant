...
def test_conversion_preserves_many_brands_seats_and_saved_branding(client, admin, identity, brand):
    account = str(identity["account"])
    branding = {"report_title": "Client results", "accent": "#204c34"}
    admin.execute("UPDATE account SET white_label=%s WHERE id=%s", (Jsonb(branding), account))
    ...
    business = client.put(path, json={"expected_type": "agency", "account_type": "business"})
    assert business.status_code == 200, business.text
    assert business.json()["account_type"] == "business" and business.json()["brand_count"] == 50
    repeat = client.put(path, json={"expected_type": "agency", "account_type": "business"})
    assert repeat.status_code == 200
    agency = client.put(path, json={"expected_type": "business", "account_type": "agency"})
    assert agency.status_code == 200, agency.text
    assert (
        admin.execute("SELECT * FROM brand WHERE account_id=%s ORDER BY id", (account,)).fetchall()
        == before_brands
    )
    assert (
        admin.execute("SELECT * FROM seat WHERE account_id=%s ORDER BY id", (account,)).fetchall()
        == before_seats
    )
    assert (
        admin.execute("SELECT white_label FROM account WHERE id=%s", (account,)).fetchone()[
            "white_label"
        ]
        == branding
    )
    ...
    assert history.json()[0]["from_type"] == "business" and history.json()[0]["to_type"] == "agency"
...
@pytest.mark.parametrize("role", ["owner", "admin", "buyer", "client_approver", "client_viewer"])
def test_conversion_role_boundary(client, admin, identity, brand, role):
    ...
    assert response.status_code == (200 if role in {"owner", "admin"} else 403), response.text
    if role not in {"owner", "admin"}:
        db = client.app.state.db
        actor = db.authenticate(session_digest(client.cookies.get("adjutant_session")))
        with pytest.raises(psycopg.errors.InsufficientPrivilege), db.transaction(actor) as conn:
            conn.execute("SELECT convert_account_type(%s,'agency','business')", (account,))
