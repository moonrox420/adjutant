...
@pytest.mark.parametrize("ip", ["127.0.0.1", "10.0.0.5", "169.254.169.254", "::1", "224.0.0.1"])
def test_dns_private_and_multicast_rejected(ip):
    with patch("adjutant.research.socket.getaddrinfo", return_value=[(2, 1, 6, "", (ip, 443))]):
...
def test_dns_rebinding_uses_validated_ip():
    with patch(
        "adjutant.research.socket.getaddrinfo", return_value=[(2, 1, 6, "", ("93.184.215.14", 443))]
    ) as resolve:
        assert public_target("https://example.com") == ("example.com", 443, "93.184.215.14")
...
def test_planner_uses_real_http_contract_and_retries_invalid_schema(plan_input):
    ...
    def transport(request):
        ...
        return httpx.Response(
            200,
            json={
                "message": {"content": json.dumps({} if len(calls) == 1 else plan_input)},
    ...
    with (
        ...
        result = OllamaPlanner("http://localhost:11434").generate("local-model", {"facts": []})
